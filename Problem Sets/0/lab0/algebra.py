    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1,(Product)) and isinstance(expr2,(Sum)):
        ...
        for s in expr2:
            e1 = list(expr1)
            e1.append(s)
            prods.append(Product(e1))
        return Sum([prod.flatten() for prod in prods])
...
e = Product([ Product([3, 4]), Sum([1,2]) ] )
print e.simplify()

expr = Sum([1, Sum([2, 3,Product([1,2,4])]), Sum([1, Sum([2, 3,Product([1,2,4])])])  ])
print expr.flatten()
print expr.simplify()
